...
const requestConf = (config) => {
  ...
  config.headers['lang'] = `${store.state.settings.language}`
  /*if (
    config.data &&
  ...
  const aesIv = process.env.VUE_APP_AES_IV;
  console.log("===参数",config.data);
  if (aesKey !== "" && aesIv !== ""&&!config.isFilterInterface) {
...
const handleData = async (response) => {
  ...
  const aesKey = process.env.VUE_APP_AES_KEY;
  const aesIv = process.env.VUE_APP_AES_IV;
  const b64Data = response.data.toString()

  let { code, data, show, msg,status,statusText=0 } = response.data;


  if (aesKey !== "" && aesIv !== ""&&!response.config.isFilterInterface) {
    const iv = CryptoJS.MD5(aesIv).toString().substr(8, 16);
    const key = CryptoJS.MD5(aesKey).toString();
  ...
  switch (code) {
    ...
    case 402:
      ...
      break;
    case 403:
      Vue.prototype.$baseMessage(errMsg, 'error', '操作无权限')
      break;
  }
  // 异常处理
