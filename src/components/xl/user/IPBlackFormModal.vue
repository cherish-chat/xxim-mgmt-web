        ...
        <el-form :model="mdl" ref="ruleForm" label-width="100px">
          ...
          <el-form-item :label="$i18n.t('用户')" prop="userId">
            <el-input
                class="ls-input"
                v-model="mdl.userId"
                :placeholder="$i18n.t('作用用户 空表示所有用户')"
                clearable
            />
          </el-form-item>
...
<script>
    import { addipblacklist,updateipblacklist } from '@/api/wt/system'
    ...
    export default {
        ...
        methods: {
            submitForm () {
                ...
                this.errorMessage = ''
                if (this.mdl.id =='') {
                    addipblacklist({userIpList:this.mdl}).then(() => {
                        // 关闭抽屉
                  ...
                  updateipblacklist( {userIpList:this.mdl}).then(() => {
                        // 关闭抽屉
                        this.handleCloseDrawer()
                        this.$Message.success(this.$i18n.t('操作成功'))
                        // 调用爷爷组件刷新数据
