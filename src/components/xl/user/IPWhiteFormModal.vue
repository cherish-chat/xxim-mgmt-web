...
<script>
    import { addusermgmtipwhitelist,updateusermgmtipwhitelist } from '@/api/wt/system'
    ...
    export default {
        ...
        methods: {
            submitForm () {
                ...
                this.errorMessage = ''
                if (this.mdl.id =='') {
                    addusermgmtipwhitelist({userIpList:this.mdl}).then(() => {
                        // 关闭抽屉
                  ...
                  updateusermgmtipwhitelist( {userIpList:this.mdl}).then(() => {
                        // 关闭抽屉
                        this.handleCloseDrawer()
                        this.$Message.success(this.$i18n.t('操作成功'))
                        // 调用爷爷组件刷新数据
