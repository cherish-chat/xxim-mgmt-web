...
<style scoped>
    .wxchat-container{ width: 100%; height: 100%; overflow: hidden;}
...
<template>
    <div class="wxchat-container" >
        <div class="window" id="window-view-container" >
            ...
            <!-- main -->
            <ScrollLoader :minHeight="minHeight"  class="container-main" v-if="dataArray && dataArray.length>0" :style="{maxHeight: maxHeight-200 + 'px'}">
                <div class="message">
                    <ul>
                        <li v-for="(message, index) in dataArray" :key="message.id" :class="message.direction==2?'an-move-right':'an-move-left'">
                            <p class="time"> <span>{{message.serverTime|formatDate}}</span> </p>
                            <p class="time system" v-if="message.content_type==10000"> <span v-html="message.content"></span> </p>
                            <div :class="'main' + (message.direction==2?' self':'')" v-else>
                                <div>
                                  <span>{{message.nickname}}</span>
                                  <img class="avatar" width="45" height="45" :src="message.avatar">
                                </div>

                                <!-- 文本 -->
<!--                                <div class="text" v-emotion="message.content" v-if="message.content_type==1001"></div>-->

                              <div class="text" v-html="message.content"></div>
                                <!-- 图片 -->
<!--                                <div class="text" v-else-if="message.content_type==1002">-->
<!--                                    <img :src="message.content" class="image" alt="聊天图片">-->
<!--                                </div>-->

                                <!-- 其他 -->
<!--                                <div class="text" v-else-if="message.content_type!=10000" v-text="'[暂未支持的消息类型:'+ message.type +']\n\r' + message.content">
