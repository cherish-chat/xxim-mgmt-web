...
<el-card class="content-container" shadow="never">
      <vab-query-form>
        <vab-query-form-left-panel :span="24">
          <el-button size="mini" v-perms="['system:apipath:add']" type="primary" icon="el-icon-plus" @click="onAdd">{{$i18n.t('添加')}}</el-button>
        </vab-query-form-left-panel>
      </vab-query-form>
        <!-- 表格 -->
            <el-table border style="width: 100%;" stripe :data="data" v-loading="loading" @selection-change="handleSelectionChange">
              <template   v-for="(item,index) in tableColumns">
                ...
                <el-table-column align="center" :label="$i18n.t('操作')" fixed="right" :width="150">
                    <template slot-scope="scope">
                        <el-link v-perms="['system:apipath:edit']" type="primary"  icon="el-icon-edit" @click="onEDit(scope.row)">{{$i18n.t('编辑')}}</el-link>
                        <span v-perms="['system:apipath:del']" >
...
<script>
    ...
    import { apipathList, apiPathDelete } from '@/api/wt/system'
    import FormModal from '@/components/xl/api/FormModal'
        ...
        computed: {
          formItems () {
            const statusOptions = [
              ...
              { label: this.$i18n.t('正常'), value: '1' },
              { label: this.$i18n.t('禁用'), value: '0' }
            ]

            // 构建搜索表单
            return [
              { title: this.$i18n.t('日志'), type: 'select', key: 'logEnable',options: statusOptions },
              { title: this.$i18n.t('t标题'), type: 'input', key: 'title' },
              { title: this.$i18n.t('路径'), type: 'input', key: 'path' }
            ]
          },
            isShowDivider () {
        ...
        methods: {
          ...
          onReset() {
            ...
            fetch () {
              ...
              let obj={...this.params}
              delete obj.time
              apipathList({ filter:obj,page: {
                    page: this.page.currentPage,
                    size: this.page.perPage
                  }}).then((res) => {
                    const { apiPaths, total } = res.data
                    this.data = apiPaths
                    this.page.total = total
                    this.page.pageCount =apiPaths?apiPaths.length:0
                }).finally(() => {
                    this.loading = false
                })
            },
