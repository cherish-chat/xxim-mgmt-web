<template>
    <div class="ip-black-container">

      <el-card class="content-container" shadow="never">
      <vab-query-form>
        <vab-query-form-left-panel :span="24">
            <el-button size="mini" v-perms="['user:ipblacklist:add']" type="primary" icon="el-icon-plus" @click="onAdd">{{$i18n.t('添加')}}</el-button>
            ...
            <el-table border style="width: 100%;" stripe :data="data" v-loading="loading" @selection-change="handleSelectionChange">
              <template   v-for="(item,index) in tableColumns">
                ...
                <el-table-column align="center" :label="$i18n.t('操作')" fixed="right" :width="150">
                    <template slot-scope="scope">
                        <el-link v-perms="['user:ipblacklist:edit']" type="primary"  icon="el-icon-edit" @click="onEDit(scope.row)">{{ $i18n.t('编辑') }}</el-link>
                        <span  v-perms="['user:ipblacklist:del']">
...
<script>
    ...
    import { ipblacklist, deleteipblacklist } from '@/api/wt/system'
    import FormModal from '@/components/xl/user/IPBlackFormModal.vue'
    ...
    export default {
        ...
        methods: {
          ...
          onReset() {
            ...
            fetch () {
                this.loading = true
              ipblacklist({ filter:this.params,page: {
            ...
            // 删除
            onDelete (ids) {
                deleteipblacklist({ids:ids}).then(() => {
