          ...
          >
            <template slot-scope="scope">
              ...
              <div v-else-if="item.prop == 'isEnable'">
                <el-tag v-if="scope.row[item.prop]" type="success">{{$i18n.t('启用')}}</el-tag>
                <el-tag v-else type="danger">{{$i18n.t('禁用')}}</el-tag>
              </div>
              <div v-else>
                {{ scope.row[item.prop] }}
              </div>
...
<script>
  ...
  import { linkLists, linkDelete, getRoleList } from '@/api/wt/system'
  import FormModal from '@/components/xl/app/link'
    ...
    data() {
      return {
        ...
        tableColumns: [
          ...
          {
            label: this.$i18n.t('链接'),
            prop: 'url',
            isHeadShow: true,
            width:250,
          },
          {
            label: this.$i18n.t('图标'),
            prop: 'icon',
            isHeadShow: true,
          },
          {
    ...
    methods: {
      ...
      fetch() {
        ...
        linkLists({
          ...
          .then((res) => {
            const { appMgmtLinks, total } = res.data
            this.data = appMgmtLinks
            this.page.total = total
            this.page.pageCount = appMgmtLinks ? appMgmtLinks.length : 0
          })
          .finally(() => {
            this.loading = false
          })
